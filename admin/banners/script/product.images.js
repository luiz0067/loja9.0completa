...
ProductImages.useImageFromGallery = {
	...
	searchProductImages: function () {
		...
		$.getJSON(
			'remote.php?remoteSection=products&w=getsourceproductimages&searchterm=' + encodeURIComponent($('#ProductImagesSearch', $('#ModalContainer')).val()),
			function(json) {
	...
	loadImagesFromSourceToContainer: function(json) {
		...
		var imageHTML = '<div class="GalleryImageContainer"><div class="galleryImage" style="background-position: center center; background-image: url(\'{image.url}\'); background-repeat: no-repeat;" id="{image.id}"><div class="overlaySelect"></div></div><span class="ProductName">{image.productname}</span><a class="ViewLarger" id="viewlarger_{image.id}" href="#" onclick="ProductImages.useImageFromGallery.onClickViewLarger(\'{image.zoom}\', {image.zoomwidth}, {image.zoomheight});">' + lang.ImageFromGalleryViewLarger + '</a></div>';
		var allImagesHTML = '';
		var image = {};
...
ProductImages.Image = function (id, productId, preview, zoom, description, baseThumbnail, sort, hash) {
	if (typeof id == 'object') {
	...
	var _id, _productId, _preview, _zoom, _description, _baseThumbnail, _sort, _hash, _row;
	...
	self.setPreview = function (preview) {
		if (_preview !== preview) {
