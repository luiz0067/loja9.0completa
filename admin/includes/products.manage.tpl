				...
				<tr>
					...
					<td nowrap>
						<form action="index.php?ToDo=viewProducts%%GLOBAL_SortURL%%" method="get" onsubmit="return ValidateForm(CheckSearchForm)" style="margin: 0; padding: 0">
							...
							<input name="searchQuery" id="searchQuery" type="text" value="%%GLOBAL_EscapedQuery%%" class="Button" size="20" />&nbsp;
	...
	<script type="text/javascript">
		...
		function ShowStock(id, InventoryType, VariationId) {
			...
			if(img.src.indexOf("plus.gif") > -1)
			{
				...
				$(trQ).find('.QuickView').load('remote.php?w=inventoryLevels&p='+id+'&i='+InventoryType+'&v='+VariationId+'&t='+tok, {'cache': false}, function() {
					trQ.style.display = "";
				});
			}
			else
