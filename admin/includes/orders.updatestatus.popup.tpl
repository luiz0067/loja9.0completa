<table id="OuterPanel" cellSpacing="0" cellPadding="0" width="100%">
	<tr>
		<td class="Heading1">
			%%LNG_OrderUpdateStatusInProgress%%
		</td>
	</tr>
	<tr>
		<td class="Intro">
...
<script type="text/javascript">
	...
	var orderidx = [%%GLOBAL_JavaScriptOrderIds%%];
	var currentOrder = 0;
	var success = 0;
	var failed = 0;
	...
	function requestOrderUpdate()
	{
		$.ajax({
			type	: "POST",
			url	: "remote.php",
			data	: "w=updateMultiOrderStatusRequest&remoteSection=orders&orderId=" + orderidx[currentOrder] + "&statusId=%%GLOBAL_StatusID%%&success=" + success + "&failed=" + failed,
	...
	function handleOrderUpdate(data)
	{
		...
		$("#ProgressBarStatus").text("%%LNG_OrderUpdateStatusUpdating%% " + orderidx.length + " %%LNG_Orders%%");
