...
if(isset($_GET['prodfield']) && isset($_GET['cartitem'])) {
	if(!isset($_SESSION['CART']['ITEMS'][$_GET['cartitem']]['product_fields'][$_GET['prodfield']]['fileName'])) {
		header( "HTTP/1.0 404 Not Found" );
		exit ;
	}

	$file =  $_SESSION['CART']['ITEMS'][$_GET['cartitem']]['product_fields'][$_GET['prodfield']];
	$imagedir = ISC_BASE_PATH.'/'.GetConfig('ImageDirectory').'/configured_products_tmp/';

	$fileName = ltrim(basename(' '.$file['fileName']));
