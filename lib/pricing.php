...
if($mos=='nao'){
   ...
   if(!$customerClass->GetCustomerId()) {
    return '<a href="%%GLOBAL_ShopPathNormal%%/login.php"><img src="%%GLOBAL_AppPath%%/modificacoes/login.png" border="0"></a>';
   } 
}
...
function CalcProductVariationPrice($basePrice, $type, $difference, $product=null)
{
	...
	if(!is_null($product)) {
		...
		$taxable = $product['prodistaxable'];
	}

	// apply any tax if applicable.
	$newPrice = CalcRealPrice($newPrice, 0, 0, $taxable);
