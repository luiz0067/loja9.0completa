...
//  function add($p_filelist, $p_add_dir="", $p_remove_dir="")
  ...
  public function delete(/* options */)
  {
    ...
    if ($v_size <= 0) {
        // ----- Error log
        self::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, "Missing arguments");
    ...
    if (   (!isset($v_options[PCLZIP_OPT_BY_NAME]))
        ...
        // ----- Error log
        self::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, "At least one filtering rule must be set");
...
// --------------------------------------------------------------------------------
      ...
      switch ($p_options_list[$i]) {
        ...
        */
          ...
          if (!function_exists($v_function_name)) {
            // ----- Error log
            self::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, "Function '".$v_function_name."()' is not an existing public function for option '".PclZipUtilOptionText($p_options_list[$i])."'");
  ...
  public function privAdd($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
    ...
    while ($v_size != 0)
    {
      $v_read_size = PCLZIP_READ_BLOCK_SIZE;
      if ($v_size < PCLZIP_READ_BLOCK_SIZE) {
        $v_read_size = $v_size;
      }
  ...
  public function privAddFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
    ...
    for ($j=0; ($j<count($p_list)) && ($v_result==1); $j++)
    {
      ...
      if (!file_exists($p_filename))
      {
        ...
        self::privErrorLog(PCLZIP_ERR_MISSING_FILE, "File '$p_filename' does not exists");
      ...
      {
        // ----- Error log
        self::privErrorLog(-5, "File name is too long (max. 255) : '$p_filename'");
...
//    $p_header['external'] = (is_file($p_filename)?0xFE49FFE0:0x41FF0010);
  ...
  function PclZipUtilCopyBlock($p_src, $p_dest, $p_size, $p_mode=0)
  {
    ...
    if ($p_mode==0)
    {
      ...
      {
        $v_read_size = PCLZIP_READ_BLOCK_SIZE;
        if ($p_size < PCLZIP_READ_BLOCK_SIZE) {
          $v_read_size = $p_size;
        }
        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 4, "Read $v_read_size bytes");
