<?php
/**
 * Basic Data Store Class for Loja Virtual V2010
 *
...
class ISC_DATA_STORE
{
	...
	public function UpdateCustomerGroupsCategoryDiscounts($groupId=0)
	{
		...
		while($categoryDiscount = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$data[$categoryDiscount['catorprodid']] = array('discountAmount' => $categoryDiscount['discountpercent'], 'discountMethod' => $categoryDiscount['discountmethod']);
			...
			if($categoryDiscount['appliesto'] == 'CATEGORY_AND_SUBCATS') {
				...
				foreach($childCategories as $childCat) {
					if(!isset($data[$childCat])) {
						$data[$childCat] = array('discountAmount' => $categoryDiscount['discountpercent'], 'discountMethod' => $categoryDiscount['discountmethod']);
