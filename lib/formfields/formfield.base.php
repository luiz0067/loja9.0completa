...
abstract class ISC_FORMFIELD_BASE
{
	...
	private function buildFormField($loadForFrontend=true, $altTemplate='')
	{
		...
		if ($loadForFrontend) {
			$template = dirname(__FILE__) . '/templates/' . $templateFile . '.frontend.html';
		} else {
			$template = dirname(__FILE__) . '/templates/' . $templateFile . '.backend.html';
		}
		...
		return $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true, $contents);
	}
	...
	protected function buildForFrontend($altTemplate='')
	{
		...
		$GLOBALS['FormFieldFieldData'] = self::buildFormField(true, $altTemplate);

		/**
		 * Load our template. If we can't then we die here
		 */
		$template = realpath(dirname(__FILE__) . '/templates/formfield.frontend.html');
	...
	protected function buildForBackend()
	{
		 ...
		 */
		if (isset($this->record['formfieldisimmutable']) && $this->record['formfieldisimmutable']) {
		...
		$GLOBALS['FormFieldTabWorkSpace'] = self::buildFormField(false);

		/**
		 * Load our template. If we can't then we die here
		 */
		$template = realpath(dirname(__FILE__) . '/templates/formfield.backend.html');
	...
	public function saveForBackend($data, &$error)
	{
		 ...
		 */
		if (!$this->isLoaded() || !isset($this->record['formfieldisimmutable']) || !$this->record['formfieldisimmutable']) {
