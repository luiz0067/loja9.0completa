...
function GetOrder($orderId, $products=true, $hardRefresh=false)
{
	...
	if ($products == true) {
		...
		$query = sprintf("SELECT *
						...
						WHERE orderorderid='%d'
						ORDER BY ordprodname  ASC",
						$GLOBALS['ISC_CLASS_DB']->Quote($orderId)
				);
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		switch($row['fieldtype']) {
			case 'file': {
				...
				if (preg_match('/image/', $row['filetype'])) {
					$fieldValue = "<img width='50' src ='".$GLOBALS['ShopPath']."/arquivos.php?orderprodfield=".$row['orderfieldid']."' />";
				}
...
function EmailInvoiceToCustomer($orderId, $newStatusId=null)
{
	...
	if($order_row['ordcustid'] > 0) {
		$GLOBALS['ViewOrderStatusMsg'] = GetLang('ASummaryIsShownBelow')." <a href='".$GLOBALS['ShopPath']."/pedidos.php'>".GetLang('ClickHere')."</a>.";
	} else {
	...
	if ($order_row['ordshipfirstname'] == "") {
		...
		$GLOBALS['ShipAddressLines'] = isc_html_escape($order_row['ordshipstreet1']);

		if ($order_row['ordshipstreet2'] != "") {
			$GLOBALS['ShipAddressLines'] .= '<br />' . isc_html_escape($order_row['ordshipstreet2']);
		}
		...
		$GLOBALS['ShipPhone'] = isc_html_escape($order_row['ordshipphone']);
	...
	foreach($order_row['products'] as $product_row) {
		...
		if($product_row['ordprodtype'] == 'digital' && OrderIsComplete($order_status)) {
			...
			$downloadLink = $GLOBALS['ShopPathSSL'].'/areadoclientewebshop.php?action=download_item&amp;data=' . $downloadEncrypted;
...
function EmailOnStatusChange($orderId, $status)
{
	...
	if ($order['ordcustid'] > 0) {
		...
		$GLOBALS['ViewOrderStatusLink'] = '<a href="'.$GLOBALS['ShopPathSSL'].'/pedidos.php">'.GetLang('ViewOrderStatus').'</a>';
	} else {
	...
	if (OrderIsComplete($status) && $numDigitalProducts > 0) {
		...
		while ($product_row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$GLOBALS['DownloadsLink'] = $GLOBALS['ShopPathSSL'].'/areadoclientewebshop.php?action=download_item&amp;data='.$DownloadItemEncrypted;
...
function CompletePendingOrder($pendingOrderToken, $status, $sendInvoice=true)
{
	...
	// Create the customer if we can
	if ($createCustomer) {
		$GLOBALS['ISC_CLASS_CUSTOMER'] = GetClass('ISC_CUSTOMER');

		// Do we have any pending customer data?
		$customerData = $GLOBALS['ISC_CLASS_CUSTOMER']->getCustomerOrderSession();
		if (is_array($customerData)) {

			$savedata = array(
				'custconemail' => $customerData['email'],
				'custpassword' => $customerData['password'],
				'custconfirstname' => $customerData['firstname'],
				'custconlastname' => $customerData['lastname'],
				'custconcompany' => $customerData['company'],
				'custconphone' => $customerData['phone'],
				'customertoken' => $customerData['token'],
			);

			// Do we have any custom fields?
			if (isset($customerData['custom']) && is_array($customerData['custom'])) {
				$savedata['custformsessionid'] = $GLOBALS['ISC_CLASS_FORM']->saveFormSessionManual($customerData['custom']);
			}

			// ToDo: How to handle a failed customer creation?
			$customerid = $GLOBALS['ISC_CLASS_CUSTOMER']->CreateCustomerAccount($savedata, true, $customerData['autoAccount']);
			...
			if (isId($customerid)) {
				if (isset($customerData['addresses']) && is_array($customerData['addresses'])) {
					...
					foreach($customerData['addresses'] as $address) {
						...
						// We kind of need the customerId aswell
						...
						if (!$shippingEntity->basicSearch($address)) {
							...
							// shipping record
							if (isset($address['shipformsessionid']) && is_array($address['shipformsessionid'])) {
								$address['shipformsessionid'] = $GLOBALS['ISC_CLASS_FORM']->saveFormSessionManual($address['shipformsessionid']);
				...
				// OK, the customer and their address have all beeen assigned, now we can log them in if this is not a
				// guest checkout auto creating thingy
				if (!$customerData['autoAccount']) {
				...
				$orderIdx = array();
				foreach ($orderData['orders'] as $order) {
					$orderIdx[] = $order['orderid'];
				}

				$GLOBALS["ISC_CLASS_DB"]->UpdateQuery("orders", $savedata, "orderid IN(" . implode(',', $orderIdx) . ")");
			}
		}

		// We need to unset the stored customer data
		$GLOBALS['ISC_CLASS_CUSTOMER']->unsetCustomerOrderSession();
	}
	...
	if($sendNotifications) {
		...
		// Do we need to add them to a Email Marketer mailing list?
		SubscribeCustomerToLists($pendingOrderToken);

		// Update the current uses of each rule
		// @todo rewrite discount rules because this won't work when the order is completed from a ping back
...
function SubscribeCustomerToLists($pendingOrderToken)
{
	...
	if(isset($order['extrainfo']['join_order_list']) || GetConfig('MailOrderListAutoSubscribe') == 1) {
		// Is Email Marketer integrated?
		if(GetConfig('MailXMLAPIValid')) {
			// Add them to the Email Marketer orders list
