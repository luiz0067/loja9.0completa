<?php
	...
	*/
	 ...
	 * @param Optionally, the vendor ID to fetch the shipping details for.
	 ...
	 */
	function GetShippingZoneIdByAddress($address, $vendorId = 0)
	{
		static $zoneIdCache = array();

		// If a vendor ID was passed, we make sure that the vendor has shipping configured for them
		// otherwise, we fall back to the default shipping for the store.
		if($vendorId > 0) {
			$vendorCache = $GLOBALS['ISC_CLASS_DATA_STORE']->Read('Vendors');
			if(!isset($vendorCache[$vendorId]) || $vendorCache[$vendorId]['vendorshipping'] != 1) {
				$vendorId = 0;
			}
		}
		...
		$cacheId = md5(strtolower(serialize($address)).$vendorId);
		...
		$query = "
			...
			WHERE zonedefault='1' AND zonevendorid='".(int)$vendorId."'
		";
		...
		if($address['shipzip']) {
			$query = "
				...
				WHERE z.zonevendorid='".(int)$vendorId."' AND z.zoneenabled='1' AND locationtype='zip' AND locationcountryid='".(int)$address['shipcountryid']."' AND '".$GLOBALS['ISC_CLASS_DB']->Quote($address['shipzip'])."' REGEXP REPLACE(REPLACE(CONCAT('^', locationvalue, '$'), '*', '.{1,}'), '?',  '.')
			";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		$query = "
			...
			FROM [|PREFIX|]shipping_zone_locations l
			INNER JOIN [|PREFIX|]shipping_zones z ON (z.zoneid=l.zoneid)
			WHERE z.zonevendorid='".(int)$vendorId."' AND z.zoneenabled='1' AND locationtype='state' AND locationcountryid='".(int)$address['shipcountryid']."' AND (locationvalueid='".(int)$address['shipstateid']."' OR locationvalueid='0')
		...
		$query = "
			...
			FROM [|PREFIX|]shipping_zone_locations l
			INNER JOIN [|PREFIX|]shipping_zones z ON (z.zoneid=l.zoneid)
			WHERE z.zonevendorid='".(int)$vendorId."' AND z.zoneenabled='1' AND locationtype='country' AND locationvalueid='".(int)$address['shipcountryid']."'
			LIMIT 1
		";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$zone = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
	...
	function GetShippingZoneInfo($vendorId=0)
	{
		$query = "
			SELECT *
			FROM [|PREFIX|]shipping_zones z
			WHERE z.zoneenabled='1' AND zonevendorid='".(int)$vendorId."'
		";
