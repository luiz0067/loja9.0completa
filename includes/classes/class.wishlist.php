	...
	class ISC_WISHLIST
	{
		...
		public function HandlePage()
		{
			...
			if(CustomerIsSignedIn()) {
				...
				$this_page = urlencode(sprintf("minhaslistas.php?%s", $get_vars));
				ob_end_clean();
		...
		private function DisplayAddWishListForm()
		{
			...
			if (isset($_POST['submit'])) {
				...
				if ($this->AddWishList($customerid, $_POST['wishlistname'], $ispublic)) {
					ob_end_clean();
					header("Location: ".$GLOBALS['ShopPath']."/minhaslistas.php");
					return true;
				}
			}
		...
		private function DisplayWishListItems($MsgDesc = "", $MsgStatus = "", $wishlistid=0)
		{
			...
			if (empty($wishlist)) {
				...
				// if user trying to view wish list which is not public
					...
					if ($wishlist['ispublic']==1) {
						$GLOBALS['PublicWishListUrl'] = GetConfig('ShopPath').'/minhaslistas.php?publicwishlist='.$wishlist['wishlisttoken'];
		...
		*/
		private function _ProductInWishList($ProductId, $WishListID)
		{
			$query = "
				SELECT count(wishlistitemid) AS num
				FROM [|PREFIX|]wishlist_items
			 	WHERE productid='".(int)$ProductId."' AND wishlistid = '".(int)$WishListID."'
			";
		...
		*/
		private function AddItemToWishList()
		{
			...
			if((isset($_GET['product_id']) && is_numeric($_GET['product_id'])) && is_numeric($wishlistid)) {
				...
				if(!$this->_ProductInWishList($product_id, $wishlistid)) {

					// Add it to the whslist
					$new_item = array(
					...
					if($GLOBALS['ISC_CLASS_DB']->Error() == "") {
						...
						$add_link = sprintf("%s/minhaslistas.php?action=add&product_id=%d", $GLOBALS['ShopPath'], $product_id);
						$this->DisplayWishListItems(sprintf(GetLang('ErrorAddingItemToWishList'), $add_link), MSG_SUCCESS, $wishlistid);
					}
				}
				else {
		...
		private function MyWishLists($MsgDesc = "", $MsgStatus = "")
		{
			...
			foreach ($wishlists as $wishlist) {
				...
				if ($wishlist['ispublic']==1) {
					...
					$GLOBALS['ShareWishListLink'] = '<a href="minhaslistas.php?action=sharewishlist&amp;wishlistid='.$GLOBALS['WishListID'].'">'.GetLang('Share').'</a>';
				} else {
		...
		*/
		private function RemoveItemFromWishList()
		{
			...
			if(isset($_GET['item_id']) && is_numeric($_GET['item_id'])) {
				...
				if (empty($row)) {
					$remove_link = $GLOBALS['ShopPath']."/minhaslistas.php?action=remove&item_id=".$item_id;
				...
				if(!$GLOBALS['ISC_CLASS_DB']->Query($query)) {
					...
					ob_end_clean();
					header("Location: ".$GLOBALS['ShopPath']."/minhaslistas.php?action=viewwishlistitems&wishlistid=".$row['wishlistid']);
					die();
				}
			}
			else {
