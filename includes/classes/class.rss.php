<?php
	...
	class ISC_RSS
	{
		...
		private function _BuildProductFeed($feedTitle, $feedDescription, $feedId, $sortField, $sortOrder, $searchTerms=array())
		{
			...
			// The magical Shopping Cart RSS feeds are actually just custom searches so pipe it off to our search function
			...
			while($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				if($product['imagefile']) {
					$product['proddesc'] = sprintf("<div style='float: right; padding: 10px;'>%s</div>%s", ImageThumb($product, ProdLink($product['prodname'])), $product['proddesc']);
				}
