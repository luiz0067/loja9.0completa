...
class ISC_OPTIMIZER_PERPAGE
{
	public $conversionPages =  array(
				...
				'Cart' => 'compras.php',
				'Order' => 'formadepagamento.php',
				'Checkout' => 'concluir.php',
			);
					...
					switch(isc_strtolower($page)) {
						...
						case 'checkout':
							...
							foreach($_SESSION['CART']['ITEMS'] as $item) {
								$prodInCart[] = $item['product_id'];
							}
							if(!in_array($row['optimizer_item_id'], $prodInCart)) {
								$noConversion = true;
							}
							break;
						case 'order':
	...
	public function getLinkScriptForConversionPage($conversionPages)
	{
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
			...
			$linkScript = preg_replace('/gwoTracker\._trackPageview.*;/', '', $trackingScript);
			return $linkScript;
		}
		return '';
