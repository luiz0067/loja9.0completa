<?php
	...
	class ISC_GENERIC_CREDITCARD extends ISC_CHECKOUT_PROVIDER
	{
			...
			if(!strtolower($_SERVER['HTTPS']) == "on") {
				...
				?>
					<script type="text/javascript">
						...
						document.location.href="<?php echo $GLOBALS['ShopPath']; ?>/concluir.php?action=confirm_order";
					</script>
				<?php
				die();
			}
		...
		public function Process3DCallBack()
		{
			...
			if($orderStatus) {
				if(CompletePendingOrder($_COOKIE['SHOP_ORDER_TOKEN'], $orderStatus)) {
					...
					echo "<script>top.location.href='".$GLOBALS['ShopPathSSL']."/formadepagamento.php';</script>";
					die();
				}
			}
		}
		...
		*/
		...
		{
			$js = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('PaymentValidation_generic.creditcard');
		...
		public function ProcessManualPayment($order, $data)
		{
			...
			if ($amount <= 0) {
				return array('amount' => 0, 'result' => false);
			}
			$order['ordgatewayamount'] = $amount;
			$orderData = array(
				'orders' => array(
					$order['orderid'] => $order
				)
