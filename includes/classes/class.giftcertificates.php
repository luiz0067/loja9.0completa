...
class ISC_GIFTCERTIFICATES
{
	...
	public function UpdateGiftCertificateInCart($itemid, $certificateData)
	{
		...
		if(isset($_SESSION['CART']['ITEMS'][$itemid])) {
			$_SESSION['CART']['ITEMS'][$itemid]['product_price'] = $certificateData['amount'];
	...
	public function GenerateGiftCertificate($certificate)
	{
		...
		$giftCertTemplate = new TEMPLATE('ISC_LANG');
		$giftCertTemplate->templateExt = 'html';
		$giftCertTemplate->frontEnd = true;
		$giftCertTemplate->SetTemplateBase(ISC_BASE_PATH.'/templates');
		...
		$GLOBALS['HeaderLogo'] = $giftCertTemplate->GetSnippet("LogoText");
		...
		$giftCertTemplate->SetTemplate("../__gift_themes/".$certificate['giftcerttemplate']);
		$certificate = $giftCertTemplate->ParseTemplate(true);
