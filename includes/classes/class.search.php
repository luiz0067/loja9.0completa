	...
	class ISC_SEARCH
	{
		...
		*/
		...
		public function DoAjaxSearch()
		{
			if (isset($_GET['search_query']) && isc_strlen($_GET['search_query']) >= 3) {
				...
				if ($showViewMoreLink) {
					$tags[] = $xmlParser->MakeXMLTag("viewmoreurl", "<a href=\"" . $GLOBALS["ShopPathNormal"] . "/buscas.php?search_query=" . urlencode($_REQUEST["search_query"]) . "\">" . GetLang("QuickSearchViewAll") . " &raquo;</a>", true);
				}
		...
		*/
		public function ShowAjaxSearchPage ()
		{
			...
			$GLOBALS["SectionPaging"] = sprintf('(%1$s %2$d de %3$d) &nbsp;&nbsp;&nbsp;', $navTitle, $page, $totalPages);
