...
/**
* The Shopping Cart checkout provider base class, used by all checkout modules
*/
...
class ISC_CHECKOUT_PROVIDER extends ISC_MODULE
{
	...
	*/
	...
	public function GetPropertiesSheet($tabId, $doHeaderRows=true, $moduleId='')
	{
		...
		if(!isset($GLOBALS['CheckoutJavaScript'])) {
		...
		foreach($this->GetCustomVars() as $id=>$var) {
			...
			$GLOBALS['PropertyBox'] = $this->_buildformitem($id, $var, true, $moduleId);
			...
			if($var['help'] != "") {
				$GLOBALS['HelpTip'] = sprintf("<img onmouseout=\"HideHelp('d%d')\" onmouseover=\"ShowHelp('d%d', '%s', '%s')\" src=\"images/help.gif\" width=\"24\" height=\"16\" border=\"0\"><div style=\"display:none\" id=\"d%d\"></div>", $help_id, $help_id, $var['name'], addcslashes($var['help'], "'"), $help_id);
			}
	...
	protected function RedirectToOrderConfirmation($reason="", $targetTop=false)
	{
		...
		if($targetTop) {
			echo "<script>top.location.href='".$GLOBALS['ShopPathSSL']."/concluir.php?action=confirm_order';</script>";
		} else {
	...
	*/
	...
	protected function checkSupportedCurrencies($currency)
	{
		...
		return in_arrayi(trim($currency), $this->supportedCurrencyCodes);
	}
}