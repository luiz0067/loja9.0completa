	...
	class ISC_PRODUCTIMAGE
	{
		...
		private function _SetImageData()
		{
			...
			if(isset($_GET['variation_id'])) {
				...
				$query = "SELECT vcimagezoom FROM [|PREFIX|]product_variation_combinations WHERE vcproductid='".$this->_prodid."' AND combinationid='".$this->_variationid."'";
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				if (!$variation = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					// Invalid variation
					exit;
				}
				$this->_variationImage = $GLOBALS['ShopPath']."/".$GLOBALS['ImageDirectory']."/".$variation['vcimagezoom'];
			}
		...
		public function ShowImage()
		{
			if($image = $this->GetImage()) {
				...
				if($this->GetCurrentImage() == 0 || $this->GetCurrentImage() == 'variation') {
					...
					$GLOBALS['PrevLink'] = sprintf("%s/imagens.php?product_id=%d&current_image=%d", $GLOBALS['ShopPath'], $this->GetProductId(), $this->GetCurrentImage()-1);
				}
				...
				if($this->GetNumImages()-1 == $this->GetCurrentImage() || $this->GetCurrentImage() == 'variation') {
					...
					$GLOBALS['NextLink'] = sprintf("%s/imagens.php?product_id=%d&current_image=%d", $GLOBALS['ShopPath'], $this->GetProductId(), $this->GetCurrentImage()+1);
				}
