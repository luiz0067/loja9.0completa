	...
	class ISC_ACCOUNT
	{
		...
		public function HandlePage()
		{
			...
			if (CustomerIsSignedIn()) {
				...
				$this_page = urlencode(sprintf("areadoclientewebshop.php?action=%s", $action));
				ob_end_clean();
		...
		*/
		private function ShowReturns()
		{
			...
			if (GetConfig('EnableReturns') == 0) {
				// Bad details
				ob_end_clean();
				header(sprintf("location:%s/areadoclientewebshop.php", $GLOBALS['ShopPath']));
				die();
			}
		...
		private function SaveNewShippingAddress()
		{
			...
			if (isset($ShippingAddress['shipfirstname']) && isset($ShippingAddress['shipaddress1'])) {
				...
				if (isId($shippingid)) {
					...
					// Database error
					ob_end_clean();
					header(sprintf("location:%s/%s", $GLOBALS['ShopPath'], 'areadoclientewebshop.php?action=add_shipping_address'));
					die();
				}
			}
			else {
					...
					if ($row['ordstatus'] == 7) {
						...
						if (GetModuleById('checkout', $checkout_object, $row['orderpaymentmodule']) && $checkout_object->getpaymenttype("text") == "PAYMENT_PROVIDER_OFFLINE") {
							...
							if (method_exists($checkout_object, 'GetOfflinePaymentMessage')) {
								...
								$GLOBALS['OrderInstructions'] = $checkout_object->GetOfflinePaymentMessage($row['orderid']);
								$GLOBALS['OrderId'] = $row['orderid'];
		...
		*/
		...
		{
			$query = sprintf("select orderprodid, ordprodid, ordprodname, ordprodtype, ordprodqty, ordprodrefunded, ordprodoptions, ordprodvariationid from [|PREFIX|]order_products where orderorderid='%f'", $GLOBALS['ISC_CLASS_DB']->Quote($OrderId));
			$Result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		}
						...
						if (count($data_vals) == 6) {
							...
							while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
								...
								if ($expired == true) {
									...
									$GLOBALS['DownloadName'] = sprintf("<a href=\"%s/areadoclientewebshop.php?action=download_item&data=%s\">%s</a>", $GLOBALS['ShopPathSSL'], $GLOBALS['DownloadItemEncrypted'], $GLOBALS['DownloadName']);
...
// fim
					...
					if ($row['ordshipfirstname'] == "") {
						...
						$GLOBALS['ShipAddressLines'] = isc_html_escape($row['ordshipstreet1']);

						if ($row['ordshipstreet2'] != "") {
							$GLOBALS['ShipAddressLines'] .= '<br />' . isc_html_escape($row['ordshipstreet2']);
						}
		...
		*/
		public function PrintInvoice()
		{
			...
			echo GeneratePrintableInvoice($_GET['order_id']);
		...
		private function AddressBook()
		{
			...
			$GLOBALS['FromURL'] = urlencode("areadoclientewebshop.php?action=address_book");
			$GLOBALS['HideAddressButton'] = "none";
			$GLOBALS['CheckoutShippingTitle'] = GetLang('YourAddressBook');
			$GLOBALS['CheckoutShippingIntro'] = sprintf("%s <a href='%s/areadoclientewebshop.php?action=add_shipping_address&amp;address_type=&amp;from=%s'>%s</a>", GetLang('AddressBookIntro1'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('AddressBookIntro2'));

			$GLOBALS['CheckoutShippingIntroNoAddresses'] = sprintf("%s <a href='%s/areadoclientewebshop.php?action=add_shipping_address&amp;address_type=&amp;from=%s'>%s</a>", GetLang('AddressBookIntro1NoAddresses'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('AddressBookIntro2NoAddresses'));
		...
		private function GetUnreorderableProducts($OrderProducts, $ProductIds)
		{
			...
			foreach ($OrderProducts as $OrderProduct) {
				...
				if($OrderProduct['ordprodtype'] == 'giftcertificate') {
					$UnreorderableProducts[$OrderProduct['orderprodid']] = sprintf(GetLang('CantReorderGiftCertificate'), 'valepresentes.php');
					continue;
				}
		...
		*/
		private function DoReorder()
		{
			...
			if(!empty($UnreorderableProducts)) {
				...
				ob_end_clean();
				header(sprintf("Location: %s/areadoclientewebshop.php?action=view_order&order_id=%s&reorder=1", $GLOBALS['ShopPath'], $OrderId));
			} else {
				ob_end_clean();
				header(sprintf("Location: %s/compras.php?action=addreorderitems&orderid=%s", $GLOBALS['ShopPath'], $OrderId));
