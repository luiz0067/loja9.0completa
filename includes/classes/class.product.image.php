...
class ISC_PRODUCT_IMAGE {
	...
	*/
	public static function randomString ($length, $selection = '0123456789abcdefghijklmnopqrstuvwxyz')
	{
		...
		while ($length) {
			$output .= substr($selection, rand(0, $selectionLength), 1);
			$length--;
		}
	...
	*/
	public static function importImage ($temporaryPath, $originalFilename, $productId, $hash = false, $moveTemporaryFile = true, $generateImages = true)
	{
		...
		if (!file_exists($finalDirectory)) {
			if (!@mkdir($finalDirectory, ISC_WRITEABLE_DIR_PERM, true)) {
	...
	*/
	public function generateResizedFile ($size, $save = true, $force = false)
	{
		...
		if (!file_exists($this->getAbsoluteSourceFilePath())) {
			throw new ISC_PRODUCT_IMAGE_SOURCEFILEDOESNTEXIST_EXCEPTION($this->getAbsoluteSourceFilePath());
		}

		$sourceImageModifiedTime = filemtime($this->getAbsoluteSourceFilePath());

		if (!$force && file_exists($resizedFilePath) && $sourceImageModifiedTime < filemtime($resizedFilePath)) {
			// resized file exists and source file is hasn't changed since it was made
		...
		if (!file_exists($resizedDirectoryPath)) {
			if (!@mkdir($resizedDirectoryPath, ISC_WRITEABLE_DIR_PERM)) {
				throw new ISC_PRODUCT_IMAGE_CREATEDIRECTORY_EXCEPTION();
			}
		}
	...
	* @return void
	*/
	public function delete ($loadFirst = true, $deleteFiles = true, &$newThumbnailId = null)
	{
		$newThumbnailId = null;
		...
		$db = $GLOBALS['ISC_CLASS_DB'];
		$db->Query("SET /*ISC_PRODUCT_IMAGE->delete*/ autocommit = 0 ");
		$db->Query("LOCK TABLES /*ISC_PRODUCT_IMAGE->delete*/ `[|PREFIX|]product_images` WRITE");

		// delete the image record
		if (!$db->Query("DELETE FROM /*ISC_PRODUCT_IMAGE->delete*/ `[|PREFIX|]product_images` WHERE imageid = " . $imageId)) {
			$db->Query("ROLLBACK /*ISC_PRODUCT_IMAGE->delete*/");
			$db->Query("UNLOCK TABLES /*ISC_PRODUCT_IMAGE->delete*/");
			throw new ISC_PRODUCT_IMAGE_DBERROR_EXCEPTION(sprintf(GetLang('ProductImageDatabaseError'), __CLASS__, __METHOD__, $db->GetErrorMsg()));
		}
		...
		$db->Query("COMMIT /*ISC_PRODUCT_IMAGE->delete*/");
		$db->Query("UNLOCK TABLES /*ISC_PRODUCT_IMAGE->delete*/");
