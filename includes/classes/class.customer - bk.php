	...
	class ISC_CUSTOMER
	{
		...
		public function CheckLogin($silent=false)
		{
			if (isset($_POST['login_email']) && isset($_POST['login_pass'])) {
				...
				$query = sprintf("select customerid, custpassword, customertoken, custimportpassword from [|PREFIX|]customers where custconemail='%s'", $GLOBALS['ISC_CLASS_DB']->Quote($email), $GLOBALS['ISC_CLASS_DB']->Quote($pass));
		...
		private function LoginCustomer($ClientRecord, $silent=false)
		{
			...
			if (isset($_SESSION['LOGIN_REDIR']) && $_SESSION['LOGIN_REDIR'] != '') {
				...
				header(sprintf("Location: %s/areadoclientewebshop.php", $GLOBALS['ShopPathNormal']));
			}
				...
				if (isId($customerId)) {
					...
					if (!isset($_SESSION['IsCheckingOut'])) {
						...
						if (isset($_SESSION['CHECKOUT']['IS_SPLIT_SHIPPING']) && $_SESSION['CHECKOUT']['IS_SPLIT_SHIPPING'] == true) {
							header("Location: " . $GLOBALS['ShopPath'] . "/concluir.php?action=multiple");
						}
						else {
							header("Location: " . $GLOBALS['ShopPath'] . "/concluir.php");
						}
		...
		public function CreateCustomerAccount($Customer, $Email=true, $checkoutAccount=false)
		{
			...
			*/
			...
			if ($Email == true) {
				...
				$GLOBALS['ISC_LANG']['ThanksForRegisteringEmailLogin'] = sprintf(GetLang('ThanksForRegisteringEmailLogin'), $GLOBALS['ShopPathSSL']."/areadoclientewebshop.php", $GLOBALS['ShopPathSSL']."/areadoclientewebshop.php", $GLOBALS['ShopPathSSL']."/areadoclientewebshop.php");
		...
		*/
		...
		/**
		 * Set the customer session used when completing the order
		 *
		 * Method will set the session that is used to store the customer data. This data will be used later on to
		 * create the customer record
		 *
		 * @access public
		 * @param array $customer The customer session data
		 * @return bool TRUE if the session was created, FALSE otherwise
		 */
		public function setCustomerOrderSession($customer)
		{
			if (!is_array($customer)) {
				return false;
				}

			$_SESSION['CHECKOUT']['NewCustomerAccount'] = $customer;

			return true;
		}

		/**
		 * Do we have a stored customer session?
		 *
		 * Method will return TRUE if there is a stored customer session, FALSE if not
		 *
		 * @access public
		 * @return bool TRUE if there is a stored customer session, FALSE if not
		 */
		public function isCustomerOrderSession()
		{
			if (isset($_SESSION['CHECKOUT']['NewCustomerAccount']) && is_array($_SESSION['CHECKOUT']['NewCustomerAccount'])) {
				return true;
			}
		...
		/**
		 * Get the stored customer session array
		 *
		 * Method will return the store customer session
		 *
		 * @access public
		 * @param array The stored customer session on success, FALSE if there is not stored session array
		 */
		public function getCustomerOrderSession()
		{
			if (!$this->isCustomerOrderSession()) {
				return false;
			}

			return $_SESSION['CHECKOUT']['NewCustomerAccount'];
		}

		/**
		 * Unset the stored customer session
		 *
		 * Method will unset the stored customer session
		 *
		 * @access public
		 * @return bool TRUE if the session was unset, FALSE if there was no session or it is still set
		 */
		public function unsetCustomerOrderSession()
		{
			if (!$this->isCustomerOrderSession()) {
				return false;
			}

			unset($_SESSION['CHECKOUT']['NewCustomerAccount']);

			return !$this->isCustomerOrderSession();
		}
	}