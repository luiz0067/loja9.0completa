...
abstract class ISC_ACCOUNTING extends ISC_MODULE
{
	...
	public function getPropertiesSheet($tabId, $customVars=array())
	{
		...
		if (isset($first["tabname"])) {
			$hasTabs = true;
			$GLOBALS["ModuleTabs"] = "";
			$GLOBALS["ModuleId"] = $this->getId();

			if (isset($_REQUEST["currentTab" . $this->getId()])) {
				$currentModuleTabId = $_REQUEST["currentTab" . $this->getId()];
			}
			...
			foreach ($customVars as $moduleTabId => $customVar) {
				...
				$GLOBALS["ModuleTabs"] .= " id=\"tab" . $this->getId() . $moduleTabId . "\" onclick=\"AdminAccountingSettings.showModuleTab(" . $moduleTabId . ", '" . addslashes($this->getId()) . "'); return false;\">" . isc_html_escape($customVar["tabname"]) . "</a></li>\n";
			}

			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("module.propertytabs");
			$GLOBALS["Properties"] .= $GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate(true);
		}
		...
		foreach ($customVars as $id => $customVar) {
			if ($hasTabs) {
				...
				$GLOBALS["ModuleTabID"] = $this->getId() . $id;
				...
				foreach ($customVar["tabitems"] as $itemId => $itemData) {
					$GLOBALS["ModuleTabHTML"] .= $this->getItemPropertieSheet($itemId, $itemData);
				}

				$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("module.propertytabcontainer");
				$propertiesHTML .= $GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate(true);

			} else {
				$propertiesHTML .= $this->getItemPropertieSheet($id, $customVar);
			}
		}
	...
	/**
	 * Build the HTML property sheet for an individual item
	 *
	 * Method will build and return the HTML property sheet for a module item
	 *
	 * @access public
	 * @param int $itemId The module item ID
	 * @param array $item The module item array
	 * @return string The module item HTML
	 */
	protected function getItemPropertieSheet($itemId, $itemData)
	{
		$html = "";
		...
		if (array_key_exists("heading", $itemData)) {
			...
			$html .= "<tr><td class='Heading2' colspan='2'>" . htmlentities($itemData["heading"]) . "</td></tr>";
		}

		$GLOBALS["PropertyBox"] = "";
		$GLOBALS["PropertyName"] = $itemData["name"] . ":";
		$GLOBALS["HelpTip"] = "";
		$GLOBALS["FieldId"] = $this->GetId()."_".$itemId;
		...
		$GLOBALS["PropertyBox"] = $this->_buildformitem($itemId, $itemData, false);
	...
	protected function getSpoolNodeData($type, $nodeId)
	{
		...
		return $_entityCache[strtolower($type)]->get($nodeId);
	}
