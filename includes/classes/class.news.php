<?php
	...
	class ISC_NEWS
	{
		...
		public function ShowNews()
		{
            ...
            $GLOBALS['Link'] = "";
			//contar total
			$querys = "select * from [|PREFIX|]news where newsvisible='1'";
			$total_usuarios = $GLOBALS['ISC_CLASS_DB']->CountResult($querys);
$pagina = $_GET['pagina'];	
$num_por_pagina = 10;
...
$inicio = ($pagina*$num_por_pagina) - $num_por_pagina;
			...
			$query = "select * from [|PREFIX|]news where newsvisible='1' order by newsdate desc LIMIT ".$inicio.",".$num_por_pagina."";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			
			
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			
			$data = isc_date('d/m', $row['newsdate']);
			
			$normalContent = strip_tags($row["newscontent"]);
            ...
            $GLOBALS['Link'] .= "<font size='3'><a href='".BlogLink($row["newsid"], $row["newstitle"])."'><b>".isc_html_escape($row["newstitle"])."</b></a> - ".$data."</font><hr>
...
$GLOBALS['Links'] = "$prev_link | $painel | $next_link";
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle('Blog');
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("noticias");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
			}
		}
