		...
		*/
		...
		private function AddToCart()
		{
			if(!isset($_REQUEST['product_id'])) {
				ob_end_clean();
				header(sprintf("Location: %s/compras.php", GetConfig('ShopPath')));
				die();
			}

			if (!(bool)GetConfig('AllowPurchasing')) {
			...
			if (GetConfig('ShowCartSuggestions')) {
				...
				ob_end_clean();
				header(sprintf("Location: %s/compras.php?suggest=%d", $GLOBALS['ShopPath'], $this->newCartItem));
				die();
			}
			else {
		...
		private function ShowRegularCart()
		{
			...
			if(isset($_REQUEST['suggest'])) {
				$cartProduct = $this->api->GetProductInCart($_REQUEST['suggest']);
		...
		private function ApplyCoupon()
		{
			...
			if($this->api->ApplyCoupon($_POST['couponcode'])) {
				// Coupon code applied successfully
				$GLOBALS['ISC_CLASS_CART']->api->UpdateCartInformation();
				ob_end_clean();
				header(sprintf("Location:%s/compras.php?coupon_applied=true", $GLOBALS['ShopPath']));
				die();
			}
			else {
		 ...
		 * @param boolean $enableRealTime Set to false to exclude real-time quotes from the shipping calculation.
		 * @return array An array of calculated shipping quotes.
		 */
		public function GetAvailableShippingMethodsForProducts($address, $products, $vendorId=0, $enableRealTime = true)
		{
			...
			// Get any shipping methods available for this zone
			$query = "
				SELECT *
			...
			while($method = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				// if this a vendor then we need to set the shipping origin location to the vendors location as this shipping module may not be defined specifically for the vendor
				if ($vendorId) {
					$shippingModule->SetOriginByVendorId($vendorId);
				}
		 ...
		 */
		public function GetAvailableShippingMethods($address, $enableRealTime = true)
		{
			$shippingQuotes = array();

			$cartProducts = $this->api->GetProductsInCartByVendor();
			...
			foreach($vendorIds as $vendorId) {
				...
				$shippingQuotes[$vendorId] = $this->GetAvailableShippingMethodsForProducts($address, $products, $vendorId, $enableRealTime);
			}
		...
		private function AddReorderItems()
		{
			...
			foreach($OrderItems as $OrdProdId => $item) {
				...
				if($item['WrapId']['all'] != 0 && !$this->api->ApplyGiftWrapping($cartItemId, 'same', $item['WrapId'], $item['WrapMessage'])) {
					$_SESSION['CART']['ERROR'] = implode('<br />', $this->api->GetErrors());
					$this->ShowRegularCart();
					exit;
				}
			...
			// update the cart to re-evaluate discount rules etc.
			GetClass('ISC_CART_API')->UpdateCartInformation();
