<?php
	...
	class ISC_PRODUCT
	{
		...
		*/
		public function GetCombinationDetails($combination)
		{

			//can't use prodcalculatedprice, because it includes the tax , so the tax would be added again
			if ($this->_prodsaleprice != 0) {
				$variationBasePrice = $this->_prodsaleprice;
			} else {
				$variationBasePrice = $this->_prodprice;
			}
			...
			if($this->GetProductCallForPricingLabel()) {
				...
				$variationPrice = CurrencyConvertFormatPrice(CalcProductVariationPrice($variationBasePrice, $combination['vcpricediff'], $combination['vcprice'], $this->_product));
			}

			$youSave = $this->_prodretailprice - CalcProductVariationPrice($this->_prodprice, $combination['vcpricediff'], $combination['vcprice'], $this->_product);
			$variationSaveAmount = '';
			if($youSave > 0) {
			...
			if($combination['vcimagestd'] != '') {
				$thumb = $GLOBALS['ShopPath']."/".GetConfig('ImageDirectory')."/".$combination['vcimagestd'];
			}
			else {
				$thumb = '';
			}
			if($combination['vcimagezoom'] != '') {
				$image = $GLOBALS['ShopPath'].'/'.GetConfig('ImageDirectory').'/'.$combination['vcimagezoom'];
			}
			else {
				$image = '';
			}
		...
		*/
		...
		static public function searchForItems($searchQuery, &$totalAmount, $start=0, $limit=-1, $sortBy="")
		{
			...
			if (isset($searchQuery["price"]) && is_numeric($searchQuery["price"])) {
				$query .= " AND p.prodcalculatedprice='" . $GLOBALS["ISC_CLASS_DB"]->Quote($searchTerms["price"]) . "'";
			} else {
				if (isset($searchQuery["price_from"]) && is_numeric($searchQuery["price_from"])) {
					$query .= " AND p.prodcalculatedprice >= '" . $GLOBALS["ISC_CLASS_DB"]->Quote($searchTerms["price_from"]) . "'";
				}

				if (isset($searchQuery["price_to"]) && is_numeric($searchQuery["price_to"])) {
					$query .= " AND p.prodcalculatedprice <= '" . $GLOBALS["ISC_CLASS_DB"]->Quote($searchQuery["price_to"]) . "'";
				}
			}
		...
		static public function buildSearchResultsHTML()
		{
			...
			foreach ($results["results"] as $product) {
				...
				$GLOBALS["ProductPrice"] = CalculateProductPrice($product);
