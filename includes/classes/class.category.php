<?php
	...
	class ISC_CATEGORY
	{
		...
		public function SetCategoryData()
		{
			...
			for($i = 0; $i < count($arrCats); ++$i) {
				...
				$query = "
					...
					WHERE catname ='".$GLOBALS['ISC_CLASS_DB']->quote($arrCats[$i])."' AND catparentid='".(int)$parentCat."'
				";
				$result = $GLOBALS['ISC_CLASS_DB']->query($query);
		...
		static public function searchForItems($searchQuery, &$totalAmount, $start=0, $limit=-1)
		{
			...
			$query .= " " . implode(" OR ", $searchPart) . " " . GetProdCustomerGroupPermissionsSQL() . " ORDER BY score DESC";
