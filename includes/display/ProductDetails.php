...

{
	 ...
	 * @var object Instance of the product class that this panel is loading details for.
...
if($mos=='nao'){
   ...
   if(!$customerClass->GetCustomerId()) {
	$GLOBALS['Orcamento'] = "display: none;";
	$GLOBALS['SNIPPETS']['ProductAddToCart'] = '<br><br><b>Acesse sua conta para poder comprar!</b>';
   }else{
...

	{
		...
		if ($this->productClass->GetFixedShippingCost() != 0) {
			...
			$GLOBALS['ShippingPrice'] = '<img src="%%GLOBAL_AppPath%%/modificacoes/frete_gratis.gif" border="0">';
		...
		// Purchasing is allowed, so show calculated at checkout
			...
			$GLOBALS['ShippingPrice'] = '<a href="#" onClick="Frete.Abrir(%%GLOBAL_ProductId%%);"><img src="%%GLOBAL_AppPath%%/modificacoes/frete.gif" border="0"></a>';
...

	{
		...
		//modificacao aqui do orçamento
		if($this->productClass->GetProductCallForPricingLabel()) {
       	$GLOBALS['Orcamento'] = "display: none;";
		$id_real = (int)$GLOBALS['ISC_CLASS_PRODUCT']->GetProductId();
$GLOBALS['ProductPrice'] = '<a href="#" onClick="Orcamento.Abrir('.$id_real.');">SOLICITAR ORCAMENTO</a>';
		}
		//fim

		// If prices are hidden, then we don't need to go any further

		else if($this->productClass->ArePricesHidden() || !GetConfig('ShowProductPrice')) {
		...
		$finalPrice = $this->productClass->GetFinalPrice();

		$retailPrice = $this->productClass->GetRetailPrice();
		...
		if ($finalPrice < $retailPrice) {
			...
			if($this->productClass->GetProductCallForPricingLabel()) {
				...
				$GLOBALS['RetailPrice'] = "<strike>".CurrencyConvertFormatPrice($retailPrice)."</strike>";
	...
	if(!isset($GLOBALS['ISC_CLASS_PRODUCT']) || !$GLOBALS['ISC_CLASS_PRODUCT']->hasVideos()) {
		...
		$GLOBALS['MostraVideo'] = "None";

	}else{

		$GLOBALS['MostraVideo'] = "Block";
	 ...
	 * Do we want to show a taxable label for this product?

	 */

	private function SetTaxLabel()

	{

		// Call for pricing is enabled, don't show the tax label
		...
		if(GetConfig('TaxTypeSelected') > 0 && $this->productClass->GetProductTaxable()) {
...

	{
		...
		if ($GLOBALS['NumProdImages']) {
			...
			//$GLOBALS['ProductThumbURLs'] = array();
			...
			foreach ($productImages as $productImage) {
				...
				if(GetConfig('ProductImagesTinyThumbnailsEnabled')==1) {
					...
					if(GetConfig('ProductImageMode') == 'lightbox') {
						...
						$GLOBALS['TinyImageClickJavascript'] = "showProductImage('".GetConfig('ShopPath')."/imagens.php', ".$GLOBALS['ProductId'].", ".$i.");";
				...
				if(GetConfig('ProductImagesImageZoomEnabled') == 1) {
					...
					if($resizedZoomDimension[ISC_PRODUCT_IMAGE_DIMENSIONS_WIDTH]<300 && $resizedZoomDimension[ISC_PRODUCT_IMAGE_DIMENSIONS_HEIGHT]<300) {
		...
		$GLOBALS['TinyImageClickJavascript'] = "showProductImage('".GetConfig('ShopPath')."/imagens.php', ".$GLOBALS['ProductId'].");";
...

	{
		...
		if (!GetConfig('ShowInventory') || $this->productClass->GetProductInventoryTracking() == 0) {
