 ...
 */
class ISC_HOMETAGS_PANEL extends PANEL
{
	/**
	...
	public function SetPanelSettings()
	{
		...
		while($tag = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			if($max > $min) {
				$fontSize = (($weight/80) * ($max - $min)) + $min;
			}
			else {
				$fontSize = (((80-$weight)/80) * ($max - $min)) + $max;
			}
			...
			$GLOBALS['SNIPPETS']['TagList'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('HomeTags');
		}
	}
}