<?php
	...
	/*******************************************\
	*                                           *
	*  Generic Shopping Cart Panel Parsing Class   *
	*                                           *
	\*******************************************/
	...
	CLASS ISC_CATEGORYHEADING_PANEL extends PANEL
	{
		public function SetPanelSettings()
		{
			...
			if ($GLOBALS['EnableSEOUrls'] == 1) {
				...
				$baseLink = sprintf("%s/departamento.php?category=", $GLOBALS['ShopPath']);
			}
			...
			if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
				...
				while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					...
					if ($useImages) {
						...
						$GLOBALS['SNIPPETS']['SubCategories'] .= '<li style="width: ' . GetConfig('CategoryImageWidth') . 'px; height: ' . (GetConfig('CategoryImageHeight')+50) . 'px;">' . $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("SubCatItemImage") . '</li>';

						if ($i % GetConfig('CategoryPerRow') == 0) {
							$GLOBALS['SNIPPETS']['SubCategories'] .= '<li style="float:none; clear:both;"></li>';
						}
			...
			if($GLOBALS['ISC_CLASS_CATEGORY']->GetNumProducts() > 1) {
				...
				if($GLOBALS['EnableSEOUrls'] == 1) {
					...
					$GLOBALS['URL'] = $GLOBALS['ShopPath']."/departamento.php";
